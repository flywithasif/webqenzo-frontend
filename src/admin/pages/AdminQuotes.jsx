...
const getActivityHistory = (quote, extraActivities = []) => {
  const sources = [
    [quote?.conversationHistory, "Comment"],
    [quote?.commentHistory, "Comment"],
    [quote?.comments, "Comment"],
    [quote?.activityHistory, "CRM Activity"],
    [quote?.activities, "CRM Activity"],
    [quote?.leadHistory, "Lead Update"],
    [quote?.history, "Lead Update"],
    [quote?.statusHistory, "Status Update"],
    [quote?.assignmentHistory, "Assignment"],
  ];
  ...
  if (quote?.updatedAt && quote?.updatedAt !== quote?.createdAt) {
    items.push({
      _id: `record-updated-${quote._id}`,
      createdAt: quote.updatedAt,
      action: "Lead Record Updated",
      message: "Quote information was updated.",
      adminName: "System",
    });
  }
...
const AdminQuotes = () => {
  ...
  const openQuote = (quote) => {
    ...
    setCrm({
      ...
      nextFollowUpDate:
        toInputDate(quote.nextFollowUpDate),
      nextFollowUpTime:
        quote.nextFollowUpTime || "",
      doNotFollowUp:
        quote.leadQuality === "COLD" &&
  ...
  const saveQuote = async () => {
    ...
    if (
      ...
      showToast(
        "Please add a reason before marking this quote cold.",
        "error"
      );
      return;
    }
    ...
    try {
      ...
      if (!updated) {
        throw new Error(
          "Quote was not returned by the server."
        );
      }
  ...
  const hotCount = quotes.filter(
    (item) => item.leadQuality === "HOT"
  ).length;

  const warmCount = quotes.filter(
    (item) => item.leadQuality === "WARM"
  ).length;

  const coldCount = quotes.filter(
          ...
          <div className="relative flex flex-col gap-6 px-5 py-6 sm:px-7 sm:py-7 lg:flex-row lg:items-center lg:justify-between">
            ...
            <div>
              ...
              <p className="mt-2 max-w-2xl text-sm leading-6 text-slate-500">
                Manage website quote requests, assigned leads,
                conversations, assignments and follow-ups from one
          ...
          <div className="flex flex-col gap-3 xl:flex-row">
            ...
            <div className="relative flex-1">
              ...
              <input
                ...
                placeholder="Search by name, mobile, email, company, service or lead type..."
          ...
          {loading ? (
            ...
            <div className="flex min-h-[360px] flex-col items-center justify-center px-6 text-center">
              ...
              <p className="mt-1 max-w-sm text-sm leading-6 text-slate-400">
                Try changing your search, quality or service filter.
              </p>
            </div>
          ) : (
                  ...
                  <section className="border border-slate-200 bg-white">
                    ...
                    <div className="grid sm:grid-cols-2">
                      ...
                      <div className="border-b border-slate-100 px-5 py-4 sm:border-r">
                        <p className="text-[9px] font-bold uppercase tracking-[0.13em] text-slate-400">Lead Name</p>
                        <p className="mt-1.5 text-sm font-bold text-slate-800">
                          {selected.fullName || "Unnamed Lead"}
                        </p>
                      </div>

                      <div className="border-b border-slate-100 px-5 py-4">
                        <p className="text-[9px] font-bold uppercase tracking-[0.13em] text-slate-400">Email</p>
                      ...
                      <div className="border-b border-slate-100 px-5 py-4 sm:border-r">
                        <p className="text-[9px] font-bold uppercase tracking-[0.13em] text-slate-400">Mobile</p>
                      ...
                      <div className="border-b border-slate-100 px-5 py-4">
                        <p className="text-[9px] font-bold uppercase tracking-[0.13em] text-slate-400">Service</p>
                        <p className="mt-1.5 text-sm font-semibold text-slate-700">
                      ...
                      <div className="border-b border-slate-100 px-5 py-4 sm:border-r">
                        <p className="text-[9px] font-bold uppercase tracking-[0.13em] text-slate-400">Company</p>
                      ...
                      <div className="border-b border-slate-100 px-5 py-4">
                        <p className="text-[9px] font-bold uppercase tracking-[0.13em] text-slate-400">Budget</p>
                        <p className="mt-1.5 text-sm font-semibold text-slate-700">
                      ...
                      <div className="border-b border-slate-100 px-5 py-4 sm:border-r">
                        <p className="text-[9px] font-bold uppercase tracking-[0.13em] text-slate-400">Timeline</p>
                        <p className="mt-1.5 text-sm font-semibold text-slate-700">
                      ...
                      <div className="border-b border-slate-100 px-5 py-4">
                        <p className="text-[9px] font-bold uppercase tracking-[0.13em] text-slate-400">Existing Website</p>
                        <p className="mt-1.5 break-words text-sm font-semibold text-slate-700">
                          {selected.existingWebsite || "Not provided"}
                        </p>
                      </div>
                    </div>
                  </section>
...
const ActivityPreview = ({ quote, extraActivities = [] }) => {
  const history = getActivityHistory(quote, extraActivities).slice(0, 4);
    ...
    return (
      <div className="flex min-h-[180px] flex-col items-center justify-center px-5 text-center">
        ...
        <p className="mt-1 max-w-xs text-[10px] leading-5 text-slate-400">
          Quote activity, comments and status updates will appear here.
        </p>
      </div>
    );
  }
...
const HistoryModal = ({ quote, extraActivities = [], onClose }) => {
  const history = getActivityHistory(quote, extraActivities);
      ...
      <div className="flex h-full max-h-screen w-full flex-col overflow-hidden border border-slate-800 bg-white shadow-[0_35px_100px_rgba(0,0,0,0.35)] sm:h-[90vh] sm:max-w-[760px]">
        <div className="flex shrink-0 items-center justify-between border-b border-slate-200 bg-[#07101f] px-5 py-4 text-white sm:px-6">
          <div className="flex min-w-0 items-center gap-3">
            ...
            <div className="min-w-0">
              <p className="text-[9px] font-bold uppercase tracking-[0.17em] text-cyan-400">Complete Quote History</p>
              <h3 className="mt-0.5 truncate text-base font-bold">{quote.fullName || "Quote Activity"}</h3>
            </div>
          </div>
          <button
            type="button"
